...
import (
	...
	"github.com/aaronwong1989/gosms/comm/container"
...
const ConfigKeyPrefix = "_config_key_prefix_"
...
func CreateYamlFactory(fileName ...string) YmlConfig {
	...
	// 配置文件所在目录
	yamlConfig.AddConfigPath(BasePath() + "/config")
	// 需要读取的文件名,默认为：config
	if len(fileName) == 0 {
	...
	y.viper.OnConfigChange(func(changeEvent fsnotify.Event) {
		if time.Now().Sub(lastChangeTime).Seconds() >= 1 {
			if changeEvent.Op.String() == "WRITE" {
				y.clearCache()
				lastChangeTime = time.Now()
				log.Infof("[%-9s] config file changed, reload!", "Config")
			}
		}
	})
	y.viper.WatchConfig()
...
func (y *ymlLoader) Clone(fileName string) YmlConfig {
	...
	if err := (&ymlC).viper.ReadInConfig(); err != nil {
		log.Fatalf("配置文件Clone失败：", zap.Error(err))
	}
	return &ymlC
}
