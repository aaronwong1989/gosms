...
func login(t *testing.T, c net.Conn) bool {
	con := telecom.NewLogin()
	t.Logf(">>>: %s", con)
	i, _ := c.Write(con.Encode())
	assert.True(t, uint32(i) == con.PacketLength)

	pl := telecom.LoginRespLen
	resp := make([]byte, pl)
	i, _ = c.Read(resp)
	assert.True(t, i == pl)

	header := &telecom.MessageHeader{}
	err := header.Decode(resp)
	if err != nil {
		return false
	}
	rep := &telecom.LoginResp{}
	err = rep.Decode(header, resp[telecom.HeadLength:])
	if err != nil {
		return false
	}
	t.Logf("<<<: %s", rep)
...
func sendMt(t *testing.T, c net.Conn) bool {
	mts := telecom.NewSubmit([]string{"13100001111"}, fmt.Sprintf("hello world! %d", rand.Uint64()), telecom.MtOptions{})
	mt := mts[0]
...
func readResp(t *testing.T, c net.Conn) bool {
	...
	if header.RequestId == telecom.CmdSubmitResp {
		csr := &telecom.SubmitResp{}
	...
	} else if header.RequestId == telecom.CmdDeliver {
		dly := &telecom.Deliver{}
	...
	} else if header.RequestId == telecom.CmdActiveTestResp {
		at := (*telecom.ActiveTest)(header)
		t.Logf("<<< %s", header)
		ats := at.ToResponse(0).(*telecom.ActiveTestResp)
...
func sendDelivery(t *testing.T, c net.Conn) bool {
	dly := telecom.NewDeliver("13700001111", "123", "hello word 中国")
