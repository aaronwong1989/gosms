...
func sendMt(t *testing.T, c net.Conn) bool {
	mts := cmpp.NewSubmit([]string{"13100001111"}, fmt.Sprintf("hello world! %d", rand.Uint64()))
	mt := mts[0]
...
func readResp(t *testing.T, c net.Conn) bool {
	...
	if header.CommandId == cmpp.CMPP_SUBMIT_RESP {
		csr := &cmpp.SubmitResp{}
	...
	} else if header.CommandId == cmpp.CMPP_DELIVER {
		dly := &cmpp.Delivery{}
	...
	} else if header.CommandId == cmpp.CMPP_ACTIVE_TEST {
		at := cmpp.ActiveTest{MessageHeader: header}
		t.Logf("<<< %s", at)
		ats := at.ToResponse(0).(*cmpp.ActiveTestResp)
...
func sendDelivery(t *testing.T, c net.Conn) bool {
	dly := cmpp.NewDelivery("13700001111", "hello word 中国", "", "")
